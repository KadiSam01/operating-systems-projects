...
int passTotal = 0;
static const int MaxPassTotal = 1000000;
...
scheduler(void)
{
  ...
  for(;;){
    ...
    if(passTotal >=MaxPassTotal){
      passTotal = 0;
      struct proc *current; //current process
       for (current = proc; current < &proc[NPROC]; current++) {
    ...
    struct proc *lowest_pass = 0;
    int found = 0;
        for(p = proc; p < &proc[NPROC]; p++) {
      acquire(&p->lock);
      if(p->state == RUNNABLE) {
        if (lowest_pass == 0 || p->pass < lowest_pass->pass){
          if(lowest_pass != 0){
            release(&lowest_pass->lock);
          }
          lowest_pass = p; 
        } else {
          release(&p->lock);
        ...
        // Not RUNNABLE; just drop the lock.
        release(&p->lock);
      }
    }
      if(lowest_pass != 0){
